<?php
/**
 * Älggården Tak AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Älggården Tak AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through al_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'alggardentak', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'alggardentak' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'alggardentak' ) ) );

		register_block_pattern_category( 'alggardentak-sections', array( 'label' => __( 'Älggården Tak AB sections', 'alggardentak' ) ) );
		register_block_pattern_category( 'alggardentak-pages', array( 'label' => __( 'Älggården Tak AB page layouts', 'alggardentak' ) ) );
	}
);
