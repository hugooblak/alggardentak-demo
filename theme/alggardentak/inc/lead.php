 ...
 * or photo. They call al_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Älggården Tak AB.
 *
 ...
 * @return string
 */
function al_sample_note() {
	return al_lead( 'ar_riktig' ) ? '' : ' <span class="al-sample">(platshållare)</span>';
}
 ...
 * @return string File name for al_lead_img().
 */
function al_lead_bild( $i ) {
	$bilder = (array) al_lead( 'bilder', array() );
 ...
 * @return string
 */
function al_ring_label() {
	$namn = al_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, al_lead( 'telefon' ) ) : 'Ring ' . al_lead( 'telefon' );
}
