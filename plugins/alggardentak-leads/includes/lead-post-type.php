...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'alggardentak-leads' ),
					'singular_name' => __( 'Lead', 'alggardentak-leads' ),
					'edit_item'     => __( 'Förfrågan', 'alggardentak-leads' ),
					'search_items'  => __( 'Search leads', 'alggardentak-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'alggardentak-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'alggardentak-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'alggardentak-leads' ),
			'npl_phone'    => __( 'Phone', 'alggardentak-leads' ),
			'npl_service'  => __( 'Needs', 'alggardentak-leads' ),
			'npl_timeline' => __( 'When', 'alggardentak-leads' ),
			'npl_zip'      => __( 'ZIP', 'alggardentak-leads' ),
			'npl_source'   => __( 'Source', 'alggardentak-leads' ),
			'npl_status'   => __( 'Status', 'alggardentak-leads' ),
			'date'         => __( 'Received', 'alggardentak-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'alggardentak-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'alggardentak-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'alggardentak-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'alggardentak-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'alggardentak-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'alggardentak-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'alggardentak-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'alggardentak-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'alggardentak-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'alggardentak-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'alggardentak-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'alggardentak-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
