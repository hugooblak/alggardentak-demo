...
add_filter(
	...
	function ( $exporters ) {
		$exporters['alggardentak-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'alggardentak-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'alggardentak-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'alggardentak-leads',
						'group_label' => __( 'Quote requests', 'alggardentak-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['alggardentak-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'alggardentak-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
